...
- **카테고리 적합성**: 메인 카테고리(AI / Blockchain / Security) 중 하나에 들어가는가? (Local LLM·Physical AI 등 세부 주제는 `tags`로 표현)
- **플로우 적합성**: 표준 발행 플로우(한/영 동시, 리서치 기반, 출처 표기)에 맞는 형태인가?

→ 어느 하나라도 **맞지 않으면(애매한 주제, 새 카테고리가 필요해 보임, 형식이 다른 글 등) 임의로 진행하지 말고 사용자에게 되묻는다**:
- 새 카테고리를 추가할지, 기존 카테고리(AI/Blockchain/Security)에 편입하고 태그로 세분화할지
