...
[Bittensor](https://bittensor.com/)는 "지능을 상품화하는 마켓플레이스"를 표방한다. Miner가 AI 출력을 생성하고, Validator가 품질을 평가하며, Yuma Consensus가 평가를 온체인으로 집계해 TAO 보상으로 연결하는 구조다.
...
[Akash](https://akash.network/)는 개발자와 AI 스타트업을 위한 저가 Docker GPU 클라우드다. 역경매 기반 마켓으로 중앙 클라우드 대비 낮은 가격을 제공하며, Docker native와 Ray 클러스터 지원으로 기존 워크로드를 그대로 옮기기 쉽다. 공급자 집중과 가용성 변동이 약점으로 꼽힌다.
