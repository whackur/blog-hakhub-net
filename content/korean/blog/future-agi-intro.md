...
| 🛡️ Protect | 내장 스캐너 18종(PII·탈옥·인젝션) + 벤더 어댑터 15종(Lakera, Presidio, Llama Guard) |
| 👁️ Monitor | OpenTelemetry 기반 트레이싱. 50여 개 프레임워크(LangChain, LlamaIndex, CrewAI)를 설정 없이 연동 |
| 🎛️ Agent Command Center | OpenAI 호환 게이트웨이. 100여 개 프로바이더, 시맨틱 캐싱. ~29k req/s, 가드레일 켜고 P99 21ms 이하 |
