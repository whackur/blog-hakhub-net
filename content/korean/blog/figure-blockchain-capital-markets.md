...
Provenance는 금융 상품 기록과 정산을 위해 설계된 분산 지분증명(PoS) 블록체인으로, Cosmos SDK 기반으로 구축됐다. KYC/AML 연계 계정 메타데이터 구조와 금융 거래 특화 모듈을 지원하며, Figure가 초기 개발에 참여했다. Figure의 주요 시스템이 기록·정산·토큰화 레이어로 Provenance를 사용한다. 차용인의 개인정보(PII) 같은 민감 데이터는 그대로 온체인에 올라가지 않는 구조다.
...
주요 특징은 다음과 같다. Figure Certificate Company가 발행하고, SEC에 등록된 fixed-income security다. 1 YLDS = 1 Certificate = $0.01 NAV 구조이고, SOFR(미국 단기 기준금리로 LIBOR를 대체한 지표) minus 35bps 기반 변동 수익이 매일 accrual된다. Provenance, Solana, Stellar 등 복수 체인에서 24/7 peer-to-peer 전송과 정산을 지원한다.
