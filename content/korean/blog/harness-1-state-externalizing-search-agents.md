...
이것이 **cognitive offloading**(인지 부하를 외부로 내리는 것)이다. 사람도 복잡한 작업을 할 때 노트, 화이트보드, 스프레드시트에 중간 상태를 적으며 작업 기억 한계를 보완한다. 검색 하네스는 에이전트의 노트 역할을 하고, 모델은 그 노트를 어떻게 채울지 결정하는 데 집중한다. 결과적으로 RL은 "무엇을 추가로 검색할지, 이미 수집한 것을 어떻게 정리할지"라는 의미적 선택만 최적화하면 된다.
...
2. **Compact derived-state rendering**: evidence graph, 중요도 태그, summary, budget marker, BM25 sentence compression으로 prompt 상태를 압축한다. 문서 full-text는 바깥 계층에 두고 필요할 때만 불러온다.
3. **Diversity-preserving incentives**: diversity reward 없으면 policy가 fan_out_search 반복으로 붕괴되고 curated recall이 정체된다. 논문 기준으로 이 보상이 없을 때 tool diversity가 약 6에서 약 3.5로 떨어진다.
...
베이스라인 Context-1과 비교하면 source-family 평균 gain은 +7.9pt인 반면 held-out transfer 평균은 +17.0pt다. 이 2.2배 격차를 논문은 "도메인별 패턴 암기가 아니라 명시적 검색 상태 위의 일반적 조작 학습"의 근거로 해석한다.
...
- 실험은 evidence-seeking retrieval 중심이며, open-ended report generation, 증거가 없을 때 답을 보류하는 abstention, adversarial web 환경은 주 범위 밖이다
