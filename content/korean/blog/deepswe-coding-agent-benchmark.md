...
채점에 쓰이는 테스트는 held-out이라 에이전트에게 공개되지 않습니다. 에이전트는 지시문만 보고 어떤 동작이 검증될지 스스로 추론해야 하고, 테스트 코드에 맞춰 구현을 끼워 맞추는 식의 지름길이 막혀 있습니다.
...
DeepSWE 점수를 읽을 때는 세 가지를 기억해야 합니다. 첫째, 모든 점수가 mini-swe-agent라는 동일한 scaffold에서 나온 모델 비교라서, Claude Code나 Codex 같은 제품형 에이전트의 실사용 성능과는 다를 수 있습니다. 둘째, 채점이 행동 기반이므로 참조 정답을 외웠는지는 점수에 거의 영향을 주지 않고, held-out 테스트가 요구하는 동작을 실제로 구현해야 합니다. 셋째, 113개 태스크 중 106개가 기능 추가라서 버그 수정 위주의 SWE-bench Verified와는 측정하는 능력 자체가 다릅니다. 두 벤치마크 점수를 같은 축에 놓고 비교하면 안 됩니다.
